...
Public Class Test_NumReg2
    ...
    Public Function invKin(ByVal x As Double, ByVal y As Double, ByVal z As Double, ByVal alpha As Double)
        ...
        Dim l2 As Double = 20
        Dim l3 As Double = 290
        Dim l4 As Double = 78

        ' Define pi for conversion
        Dim pi As Double = 3.14159265359
        Dim theta(5) As Double
        '''''''''''''''''''' Modify this part'''''''''''''''''''''
        theta(1) = Math.Atan2(y, x)
        Dim r As Double = Math.Sqrt(x ^ 2 + y ^ 2)
        Dim s As Double = z + l4
        Dim A As Double = l1
        Dim B As Double = Math.Sqrt(l2 ^ 2 + l3 ^ 2)
        Dim F As Double = (r ^ 2 + s ^ 2 - A ^ 2 - B ^ 2) / (2 * A)

        Dim c3 As Double = (x ^ 2 + y ^ 2 + (z + l4) ^ 2 - (l1 ^ 2 + l2 ^ 2 + l3 ^ 2)) / (2 * l1 * Math.Sqrt(l2 ^ 2 + l3 ^ 2))
        Dim s3 As Double = -Math.Sqrt(1 - c3 ^ 2)
        theta(3) = Math.Atan2(s3, c3)

        Dim M As Double = A + B * Math.Cos(theta(3))
        Dim N As Double = B * Math.Sin(theta(3))

        Dim XX As Double = (r * M + s * N) / (M ^ 2 + N ^ 2)
        Dim YY As Double = (s - N * XX) / M

        theta(2) = -Math.Atan2(YY, XX)
        theta(4) = theta(2) - theta(3)

        theta(1) = theta(1) * 180 / pi
        theta(2) = theta(2) * 180 / pi
        theta(3) = theta(3) * 180 / pi
        theta(4) = theta(4) * 180 / pi
        theta(5) = 180 + theta(1) - alpha


        '''''''''''''''' End of modification''''''''''''''''''''''

        ' Return the array of joint angles in degrees
        Return theta
